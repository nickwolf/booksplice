...
public sealed class BookAnalyzer
{
  ...
  public async Task<BookAnalysis> AnalyzeAsync(string inputPath, bool chaptersEnabled, BookAnalysisOptions? options = null, CancellationToken cancellationToken = default)
  {
    ...
    if (discovery.Files.Count == 0)
    {
      ...
      return Empty(BookAnalysisStatus.Invalid, root, diagnostics);
    }
    ...
    if (candidate is null)
    {
      ...
      return Empty(BookAnalysisStatus.NeedsDecision, root, diagnostics, resolution);
    }
    ...
    if (candidate.FileIds.Any(id => !byId.ContainsKey(id)) || candidate.FileIds.Distinct(StringComparer.Ordinal).Count() != discovery.Files.Count)
    {
      ...
      return Empty(BookAnalysisStatus.Invalid, root, diagnostics, resolution);
    }
    ...
    return new BookAnalysis(status, root, ordered, resolution, metadata, cover, chapters, diagnostics);
  }
  ...
  private static BookAnalysis Empty(BookAnalysisStatus status, string root, IEnumerable<AnalysisDiagnostic> diagnostics, OrderResolution? resolution = null)
    => new(status, root, [], resolution, new BookMetadata(new Dictionary<SemanticField, AggregatedValue>(), new Dictionary<string, string>(), new Dictionary<SemanticField, IReadOnlyList<string>>()), new CoverDiscoveryResult([], [], null), ChapterPlan.Valid([], 0), diagnostics);
  private static string SourceRoot(string inputPath)
  {
