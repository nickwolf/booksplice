...
Private real-world acceptance remains incomplete. Before publishing 0.1.0, use disposable copies to confirm a representative long book, a very long book, a source on another physical drive, and an M4B open/save cycle in Mp3tag. The third-party binary license inventory for the pinned FFmpeg build is generated at `licenses/FFmpeg-components.md` by `scripts/Get-FFmpegLicenseInventory.ps1`. Run it with `-Check` against the exact extracted tools and resolve the report's stated vendor, platform, transitive-dependency, patent, and custom-license review items before publication. Record only anonymous outcomes. Do not commit titles, source paths, media, or command output containing private paths.
