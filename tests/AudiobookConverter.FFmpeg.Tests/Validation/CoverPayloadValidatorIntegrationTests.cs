...
using AudiobookConverter.FFmpeg.Tests.Execution;
...
public sealed class CoverPayloadValidatorIntegrationTests
{
  ...
  public async Task ValidateAsyncAcceptsTheExactAttachedCoverPayloadReadByTagLib()
  {
    ...
    var tools = new MediaToolLocator(Environment.GetEnvironmentVariable("AUDIOBOOKCONVERTER_FFMPEG_DIR")!).Resolve();
  ...
  {
    public TemporaryDirectory() => Path = Directory.CreateTempSubdirectory("audiobookconverter-cover-validation-").FullName;
