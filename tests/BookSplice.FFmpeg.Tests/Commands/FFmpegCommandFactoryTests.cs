...
public sealed class FFmpegCommandFactoryTests
{
  ...
  public void CreateFinalProducesCompleteArgumentArraysForAllStrategies()
  {
    ...
    Assert.Equal(prefix.Concat(["-i", @"C:\input\one.mp3", "-map", "0:a:0", "-c:a", "copy", "-movflags", "+faststart+disable_chpl", "-f", "ipod", "-y", @"C:\temp\copy-one.m4b"]), Factory().CreateFinal(Plan(AudioStrategy.AacStreamCopy), @"C:\temp\copy-one.m4b").Arguments);
    Assert.Equal(prefix.Concat(["-f", "concat", "-safe", "0", "-i", @"C:\temp\concat.txt", "-map", "0:a:0", "-c:a", "copy", "-movflags", "+faststart+disable_chpl", "-f", "ipod", "-y", @"C:\temp\copy-many.m4b"]), Factory().CreateFinal(Plan(AudioStrategy.AacStreamCopy, 2), @"C:\temp\copy-many.m4b", @"C:\temp\concat.txt").Arguments);
    Assert.Equal(prefix.Concat(["-i", @"C:\input\one.mp3", "-map", "0:a:0", "-c:a", "aac", "-profile:a", "aac_low", "-b:a", "96k", "-ar", "44100", "-ac", "2", "-movflags", "+faststart+disable_chpl", "-f", "ipod", "-y", @"C:\temp\direct.m4b"]), Factory().CreateFinal(Plan(AudioStrategy.DirectTranscode), @"C:\temp\direct.m4b").Arguments);
    Assert.Equal(prefix.Concat(["-i", @"C:\input\one.mp3", "-i", @"C:\input\later1.mp3", "-/filter_complex", @"C:\temp\filter.txt", "-map", "[aout]", "-c:a", "aac", "-profile:a", "aac_low", "-b:a", "96k", "-ar", "44100", "-ac", "2", "-movflags", "+faststart+disable_chpl", "-f", "ipod", "-y", @"C:\temp\filter.m4b"]), Factory().CreateFinal(Plan(AudioStrategy.FilterConcatTranscode, 2), @"C:\temp\filter.m4b", filterGraph: @"C:\temp\filter.txt").Arguments);
    Assert.Equal(prefix.Concat(["-f", "concat", "-safe", "0", "-i", @"C:\temp\segments.txt", "-map", "0:a:0", "-c:a", "copy", "-movflags", "+faststart+disable_chpl", "-f", "ipod", "-y", @"C:\temp\segmented.m4b"]), Factory().CreateFinal(Plan(AudioStrategy.SegmentedTranscode, 2), @"C:\temp\segmented.m4b", @"C:\temp\segments.txt").Arguments);
  }
