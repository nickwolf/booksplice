...
public sealed class FFmetadataWriterTests
{
  ...
  [Fact]
  public void Mp4MetadataWriterTreatsGenericAlbumArtistAliasAsNative()
  {
    var plan = Mp4MetadataWriter.CreatePlan(new Dictionary<string, string> { ["album_artist"] = "Author" });

    Assert.Equal("Author", plan.NativeFields["album_artist"]);
    Assert.Empty(plan.FreeformFields);
  }
}
